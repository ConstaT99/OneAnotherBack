...
describe('addTagFunction test', () => {
    it('the result should be true if the Tag is successfully created', async () => {
        ...
        //@ts-ignore
        expect(docRef.data().name).to.equal('covid-19');
        //@ts-ignore
        expect(docRef.data().DocId).contain('wR9n7Q6vF5i1kaep0tuq');
    ...
    it('the result should not be created if the name are the same', async () => {
        ...
        addTagFunc(testData).then(()=>{
            expect.fail('should not create duplicate tag')
        }).catch(()=>{}); //unix timestamp in seconds
    });
});